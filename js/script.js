function nextStep(step) {
  ...
  const formSteps = document.getElementById('form-steps'); // เข้าถึง container 'form-steps'
  const steps = formSteps.querySelectorAll('.step'); // เข้าถึงทุก 'step' ภายใน 'form-steps'

  steps.forEach(stepIndicator => {
    const indicatorStep = parseInt(stepIndicator.getAttribute('data-step'));
    if (indicatorStep <= step) {
      stepIndicator.classList.add('active'); // เพิ่ม 'active' ถ้า step น้อยกว่าหรือเท่ากับ step ปัจจุบัน
    } else {
      stepIndicator.classList.remove('active'); // ลบ 'active' ถ้า step มากกว่า step ปัจจุบัน
    }
  });
}
